"""Lector de datasets CRM desde la zona raw basada en Parquet."""
...
class FilesystemCrmRawReader:
    """Lee una corrida CRM ya extraida desde el sistema de archivos."""
    ...
    def prepare(self) -> None:
        ...
        if not manifest_path.exists():
            raise FileNotFoundError(
                f"No se encontro el manifiesto de la corrida CRM: {manifest_path.as_posix()}",
            )
    ...
    def read_customers(self) -> Iterator[list[Cliente]]:
        """Lee clientes desde archivos Parquet."""

        yield from self._read_dataset("clientes", Cliente)

    def read_companies(self) -> Iterator[list[Empresa]]:
        """Lee empresas desde archivos Parquet."""

        yield from self._read_dataset("empresas", Empresa)

    def read_customer_branches(self) -> Iterator[list[SucursalCliente]]:
        """Lee sucursales cliente desde archivos Parquet."""

        yield from self._read_dataset("sucursalescliente", SucursalCliente)
    ...
    def _resolve_latest_run_directory(self) -> Path:
        """Localiza la corrida CRM mas reciente dentro de la zona raw."""
        ...
        if not candidates:
            raise FileNotFoundError(
                f"No se encontraron corridas CRM en raw: {self._base_path.as_posix()}",
            )
        return candidates[0]
