"""Persistencia de extracciones operativas sobre el sistema de archivos."""
...
class FilesystemOperationalExtractionSink:
    """Persiste la corrida operativa en una zona raw basada en Parquet."""
    ...
    def finalize(self, summary: OperationalExtractionSummary) -> None:
        ...
        self._manifest_data["summary"] = {
            "vista_ordenes": summary.vista_ordenes,
            "vista_ordenes_chunks": summary.vista_ordenes_chunks,
            "ordenes": summary.ordenes,
            "ordenes_chunks": summary.ordenes_chunks,
            "ordenes_empresas": summary.ordenes_empresas,
            "ordenes_empresas_chunks": summary.ordenes_empresas_chunks,
            "preordenes": summary.preordenes,
            "preordenes_chunks": summary.preordenes_chunks,
            "orden_empresa_tecnicos": summary.orden_empresa_tecnicos,
            "orden_empresa_tecnicos_chunks": summary.orden_empresa_tecnicos_chunks,
        }
    ...
    def _build_dataframe(self, rows: list[dict[str, Any]]) -> pl.DataFrame:
        ...
        try:
            # Se inspecciona todo el lote para minimizar conflictos de inferencia
            # en vistas operativas con UNION ALL y tipos inconsistentes.
            return pl.from_dicts(rows, infer_schema_length=None)
        except ComputeError:
            coerced_rows = self._coerce_mixed_columns_to_string(rows=rows)
            return pl.from_dicts(coerced_rows, infer_schema_length=None)
