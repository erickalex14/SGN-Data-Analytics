"""Script de arranque para construir el mart tecnico en PostgreSQL."""
...
from novitec_dwh.contexts.technical.infrastructure.postgresql_technical_mart_repository import (
    PostgreSQLTechnicalMartRepository,
)
...
def main() -> None:
    """Ensambla dependencias y ejecuta la carga del mart tecnico."""

    configure_logging(log_file_name="etl_technical_mart.log")
    ...
    mart_repository = PostgreSQLTechnicalMartRepository(
        connection_factory=postgres_connection_factory,
        staging_schema=settings.postgres_technical_staging_schema,
    ...
    if not extraction_id:
        ...
        logger.info(
            "No se definio ETL_TECHNICAL_EXTRACTION_ID. Se usara la corrida mas reciente detectada en staging: %s.",
            extraction_id,
        )
    ...
    logger.info(
        "Resumen final de mart tecnico. Corrida: %s | Mart: %s | Informes: %s | Fotos: %s | Equipos: %s | Accesos: %s | Reglas calidad: %s | Hallazgos: %s.",
